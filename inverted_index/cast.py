...
def v2row_to_v5row(v1row, intbytes=2, index_length=INDEX_LENGTH):
    return v5.kv_to_row(v2.row_to_kv(v1row), intbytes=intbytes, index_length=index_length)
...
def v2_to_v3(v2file, v3file, intbytes=3, index_length=INDEX_LENGTH):
    with open(v2file, "r", encoding=ENCODING) as rf:
        with open(v3file, "wb") as wf:
            ...
            i = 0
            for l in rf.readlines():
                r = v2row_to_v3row(l, intbytes=intbytes,
...
def v2_to_v4(v2file, v3file, intbytes=3, index_length=INDEX_LENGTH):
    with open(v2file, "r", encoding=ENCODING) as rf:
        with open(v3file, "wb") as wf:
            ...
            i = 0
            for l in rf.readlines():
                r = v2row_to_v4row(l, intbytes=intbytes,
                                   index_length=index_length)
                wf.write(r)
                sys.stdout.write("\r" + str(i) + " ")
                sys.stdout.flush()
...
def v2_to_vp4(v2file, v3file, urlmap, intbytes=3, index_length=INDEX_LENGTH):
    ...
    with open(v2file, "r", encoding=ENCODING) as rf:
        with open(v3file, "wb") as wf:
            ...
            i = 0
            for l in rf.readlines():
                r = v2row_to_vp4row(l, mp, intbytes=intbytes,
                                    index_length=index_length)
                wf.write(r)
                sys.stdout.write("\r" + str(i) + " ")
                i += 1
...
def v2_to_vp5(v2file, v5file, urlmap, intbytes=3, index_length=INDEX_LENGTH):
    ...
    with open(v2file, "r", encoding=ENCODING) as rf:
        with open(v5file, "wb") as wf:
            ...
            i = 0
            for l in rf.readlines():
                r = v2row_to_vp5row(l, mp, intbytes=intbytes,
                                    index_length=index_length)
                wf.write(r)
                print(i)
                i += 1
...
def v2_to_v5(v2file, v5file, intbytes=3, index_length=INDEX_LENGTH):
    with open(v2file, "r", encoding=ENCODING) as rf:
        with open(v5file, "wb") as wf:
            wf.write(v3.int_to_bytes(intbytes, intbytes=2))
            wf.write(v3.int_to_bytes(index_length, intbytes=2))
            i = 0
            for l in rf.readlines():
                r = v2row_to_v5row(l, intbytes=intbytes,
                                   index_length=index_length)
                wf.write(r)
                print(i)
                i += 1
